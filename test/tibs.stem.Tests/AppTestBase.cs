...
using tibs.stem.Tests.TestDatas;
...
namespace tibs.stem.Tests
{
    ...
    /// Provides methods to easily work with <see cref="stemDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<stemTestModule>
    {
        ...
        {
            void NormalizeDbContext(stemDbContext context)
            {
        ...
        protected void UsingDbContext(Action<stemDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<stemDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<stemDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<stemDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<stemDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<stemDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<stemDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<stemDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<stemDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<stemDbContext, Task<T>> func)
        {
            T result;
