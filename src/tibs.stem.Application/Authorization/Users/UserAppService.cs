...
namespace tibs.stem.Authorization.Users
{
    ...
    public class UserAppService : stemAppServiceBase, IUserAppService
    {
        ...
        private readonly IRepository<AbpSalesCoOrinator> _abpSalesCoOrinatorRepository;
        public UserAppService(
            ...
            IRepository<AbpSalesCoOrinator> abpSalesCoOrinatorRepository
)
        {
            ...
            _abpSalesCoOrinatorRepository = abpSalesCoOrinatorRepository;
        ...
        public async Task<GetSalesCoordinator> GetSalesCoordinator(GetSalesCoordinatorInput input)
        {
            var output = new GetSalesCoordinator();

            var sal = (from a in _abpSalesCoOrinatorRepository.GetAll()
                       where a.UserId == input.UserId
                       select new SalesCoordinatorList
                       {
                           Id = a.Id,
                           CoordinatorId = a.CoordinatorId,
                           CoordinatorName = a.Coordinator.Name,
                           UserId = a.UserId,
                           Name = a.AbpUser.UserName
                       }).FirstOrDefault();

            output.Saless = sal.MapTo<SalesCoordinatorList>();
            return output;
        }

        public async Task CreateOrUpdateSalesCoordinator(SalesCoordinatorInput input)
        {
            if (input.Id != 0)
            {
                await UpdateSalesCoordinator(input);
            }
            else
            {
                await CreateSalesCoordinator(input);
            }
        }


        public async Task CreateSalesCoordinator(SalesCoordinatorInput input)
        {
                var sal = input.MapTo<AbpSalesCoOrinator>();

                await _abpSalesCoOrinatorRepository.InsertAsync(sal);
         
        }


        public async Task UpdateSalesCoordinator(SalesCoordinatorInput input)
        {
                var sal = input.MapTo<AbpSalesCoOrinator>();

                await _abpSalesCoOrinatorRepository.UpdateAsync(sal);
        }
    }
}
